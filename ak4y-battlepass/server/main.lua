...
local function getSteamId64(source)
    for _, id in ipairs(GetPlayerIdentifiers(source)) do
        if string.sub(id, 1, 6) == "steam:" then
            local hex = string.sub(id, 7)
...
ESX.RegisterServerCallback("ak4y-battlePass:getPlayerDetails", function(source, cb)
    ...
    cb({
        ...
        apiKey    = AK4Y.Server.ApiKey,
        steamid   = getSteamId64(source),
...
ESX.RegisterServerCallback("ak4y-battlePass:sendInput", function(source, cb, data)
    ...
    -- Basic format check (UUID: 8-4-4-4-12)
    if #code ~= 36 then cb(false) return end
