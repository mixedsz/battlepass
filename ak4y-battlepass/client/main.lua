...
RegisterCommand('battlepass', function()
	if openMenuSpamProtect < GetGameTimer() then 
		...
		ESX.TriggerServerCallback("ak4y-battlePass:getPlayerDetails", function(result)
			if result.discordid then
                discordAvatar = "https://cdn.discordapp.com/avatars/" .. result.discordid .. "/" .. result.discordid .. ".png"
            else
                discordAvatar = 'null'
            end
...
RegisterNetEvent('ak4y-battlepass:copyToClipboard')
AddEventHandler('ak4y-battlepass:copyToClipboard', function(text)
	SendNUIMessage({
